...
<head>
    ...
    <meta name="assistantBuilder" content="{{ request('assistantBuilder')->slug }}">
    ...
    <link href="{{ asset('media/css/vaa/app.css') }}" rel="stylesheet">

    <title>{{$assistantBuilder->title}}</title>
...
{{--    <script src="https://cdn.jsdelivr.net/npm/js-cookie@beta/dist/js.cookie.min.js"></script>--}}
    ...
    <script src="{{ asset('media/js/recruitment_assistant/recruitment_assistant.js') }}" async></script>

    @include('front.layouts._partials.general.settings-colors')

</head>
<body class="{{$school->slug}}">
@include('front.layouts._partials.body-tracking')
<header class="header-main">
    {{--  Modal  --}}
        @include('front.recruitment_assistant._partials.general.modal')
    {{--  Modal End  --}}
    <div class="logo-container py-3">
        @if (isset($settings['branding']['logo']))
            <a href="{{ route('school.home' ,  request()->tenant()->slug ) }}">
    ...
    <div class="header-headline">
        <h1>{{__($assistantBuilder->title)}}</h1>
    </div>
                    ...
                    <ul>
                        @foreach ($tabs as $order =>$tab)
                            ...
                            <li class="{{ ( $step == $tab['step'] ) ? 'active' : ''}} ">
                                <a href="javascript:void(0)" data-step="{{$prevStep['step']}}" data-route="{{ route('assistants.show' , [
                                'school'            => $school,
                                'assistantBuilder'  => $assistantBuilder,
                                'step'              => $tab['step']
                                ]) }}" onclick="app.isValid(this)"
                                   data-message="{{$prevStep['error_message']}}">{{$order + 1 }}</a>
...
<footer>
    <p>{{date('Y')}} {{__('All Rights Reserved by')}}
        <a href="{{$settings['school']['website']}}" target="_blank">
            {{$school->name}}.
        </a>
    </p>
</footer>
