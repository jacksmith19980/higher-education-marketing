@php
    if($campus){
        $campus = App\Tenant\Models\Campus::whereSlug($campus)->first();
    }
@endphp

@if(count($datesData) > 0)

    <input type="hidden" name="{{$field->name}}[start_date]" value="" />
    <input type="hidden" name="{{$field->name}}[end_date]" value="" />
    <input type="hidden" name="{{$field->name}}[schedule]" value="" />
    ...
    @foreach($datesData as $date)
        @if(!$campus || $campus->id == $date['date_campus'] || $date['date_campus'] == 0 )
        <div class="col-md-6 col-12">
            ...
            <div class="custom-control custom-radio date_type" style="margin-top: 7px;">
                <input class="custom-control-input radio_date bb"
                        ...
                        id="{{$date['start_date']}}_{{$date['date_schudel']['label']}}"
                        type="radio"
