@if($campus)
    <div class="col-md-6">
        @include('back.layouts.core.forms.select', [
            ...
            'label'       => 'Campus',
            'class'       => 'ajax-form-field program-campus-field' ,
            'required'    => true,
            'attr'        => '',
    ...
    <div>
        <input type="hidden" name="{{$field->name}}[campusTitle]" value="{{$campuses[$campus]}}" />
    </div>
...
@endphp
    <div class="col-md-12">
        <div class="program-start-date row form-group bb">
                ...
                if (!is_null($program->properties['dates_type'])) {
                    ...
                    switch ($program->properties['dates_type']) {
                        case 'specific-dates':
                            ...
                            if (isset($program->properties['start_date'][$i]) && strtotime($program->properties['start_date'][$i]) >= strtotime(date('Y-m-d'))) {
            ...
            @if($program->properties['dates_type'] == 'specific-dates')
                ...
                @include('front.applications.application-layouts.oiart.programs.partials.specific_dates', [
                            'datesData' => $datesData,
                            'campus'    => isset($campus) ? $campus : null,
                            'program' => $program,
                            'field' => $field,
            ...
            @elseif($program->properties['dates_type'] == 'specific-intakes')
                @include('front.applications.application-layouts.oiart.programs.partials.intake_dates', [
                    'datesData' => $datesData,
                    'program' => $program,
                    'field' => $field,
                    'format_date' => $format_date,
    ...
    @php
        ...
        if (array_key_exists('addons', $program->properties)) {
            foreach(array_keys($program->properties['addons']) as $i) {
                $addonsData[] = [
        ...
        @if(isset($params['value']['program_addons']))
            <div class="col-md-12">
                <div class="program-addons row form-group">
                        @include('front.applications.application-layouts.oiart.programs.partials.addons', [
...
@else
    ...
    @php
        $addonsData = [];
        if (array_key_exists('addons', $program->properties)) {
            foreach(array_keys($program->properties['addons']['addon_options']) as $i) {
                $addonsData[] = [
                    'addon_options_category' => $program->properties['addons']['addon_options_category'][$i],
                    'addon_options' => $program->properties['addons']['addon_options'][$i],
                    'addon_options_price' => $program->properties['addons']['addon_options_price'][$i]
...
@endif
    <div class="col-md-12 custom-fields">
        @if(isset($field->properties['customFields']))
            @include('front.applications.application-layouts.oiart.programs.partials.custom-fields')
        @endif
    </div>
