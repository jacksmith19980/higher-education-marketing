        ...
        <div id="{{$key}}" style="display: flex; flex-wrap: wrap;" class="col-md-12">
            ...
            <div class="col-md-12">
                <div class="start-date-{{$key}} row form-group">
                    ...
                    @endphp
                    @include('front.applications.application-layouts.iframe.courses.partials.dates', [
                                'datesData' => $datesData,
                                'course' => $course,
                                'field' => $field,
                                'hash' => $key,
                                'selected' => $params['value']['date'][$course_slug]
                            ])
                </div>
            </div>
            @if(array_key_exists('course_addons', $params['value']) && array_key_exists($course_slug, $params['value']['course_addons']) )
                <div class="col-md-12">
                    <div class="course-addons-{{$key}} row form-group">
                        @include('front.applications.application-layouts.iframe.courses.partials.addons', [
            ...
            @if(array_key_exists('addons', $date->properties) && !empty($date->properties['addons']) )
                <div class="col-md-12">

                    <div class="date-addons-{{$key}} row form-group">
                        @include('front.applications.application-layouts.iframe.courses.partials.dates-addons', [
            ...
            <div class="col-md-12">
                <div class="start-date-{{$key}} row form-group">
                    ...
                    @include('front.applications.application-layouts.iframe.courses.partials.dates', [
                                ...
                                'field' => $field,
                                'hash' => $key,
                                'selected' => array_key_exists($course_slug, $params['value']['date']) ? $params['value']['date'][$course_slug] : ''
                            ])
                </div>
            </div>
            @if(array_key_exists('course_addons', $params['value']) && array_key_exists($course_slug, $params['value']['course_addons']) )
