        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ $route }}" class="" aria-label=""
                    ...
                    >
                        @if (isset($get_variables) && count($get_variables) > 0)
                            @foreach($get_variables as $key => $value)
                                <input type="hidden" name="{{$key}}" value="{{$value}}">
                            @endforeach
                        @endif

                        @if($application->sections->count() == 1)
                                        ...
                                        @if ($section->fields_order)
                                            ...
                                            if(isset($preview)) {
                                                ...
                                                @if (!$field->repeater && !in_array($field->properties['type'] , ['loop' , 'filesList'])  )
                                                    ...
                                                    @include('front.applications.application-layouts.iframe.'.$field->field_type.'.'.$field->properties['type'] , $params)

                                                @endif

                                                @if ($field->properties['type'] == 'loop')
                                                    @include('front.applications.application-layouts.iframe.helper.repeater',[
                                                        'section'      => $section,
                                                        'props'   => $params,
                                                ...
                                                @if ($field->properties['type'] == 'filesList')
                                                    @include('front.applications.application-layouts.iframe.file_list.file_list',[
                                                    'section' => $section,
                                                    'props' => $params,
                                                    'submission'   => isset($submission) ? $submission : '',
                                                    ])
                        ...
                        <div class="text-right col-12">
                            <button type="submit" class="btn btn-secondary">{{isset($application->properties['save_text']) ? __($application->properties['save_text']) : __('Save')}}</button>
                        </div>
                    </form>
                </div>
            </div>
