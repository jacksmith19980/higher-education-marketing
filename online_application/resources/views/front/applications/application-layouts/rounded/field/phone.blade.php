...
<div class="field_wrapper {{($properties['smart'])? 'smart-field ' : ''}} col-md-{{$properties['wrapper']['columns']}} col-sm-12 col-xs-12 {{$properties['wrapper']['class']}}"
     {{ (!$properties['show'])? 'data-hidden=true' : ' ' }}
...
<div class="form-group">
    ...
    <input id="{{$name}}"
            ...
            data-disable="{{(isset($readOnly)) ? $readOnly : ''}}"

            @if(isset($params['country_code']))
                data-country-code="{{$params['country_code']}}"
            @elseif ($defaultValue && is_array($defaultValue))
                data-country-code="{{$defaultValue['country_code_phone']}}"

                @if (isset($defaultValue['countrycode_phone']))
                    data-countrycode-number="{{$defaultValue['countrycode_phone']}}"
                @endif
            ...
            @php
                if (is_array($defaultValue)) {
                    $defaultValue = $defaultValue['phonefield_phone'];
                }
            @endphp

            value="@if($defaultValue){{ $defaultValue }}@else{{ old($name) }}@endif"

           @if(!$properties['label']['show']) placeholder="{{__($properties['placeholder'])}}"
           placeholder="{{$properties['placeholder']}} @if(isset($properties['validation']['required']))  *@endif" @endif
