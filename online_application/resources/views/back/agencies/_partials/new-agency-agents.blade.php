<div class="card-body">
    ...
    <h4 class="title mb-2">{{ __('Invite Agents') }}</h4>
    ...
    <form method="POST" action="{{ route('agencies.inviteAgents', $agency) }}" class="">
        ...
        <input type="submit" value="Invite" class="btn btn-success float-right" />
    </form>
    ...
    @if ($agency->agents()->count())
        <h4 class="title mb-2">{{ __('Agents List') }}</h4>
                        ...
                        <tr data-row="{{ $agencyAgent->id }}">
                            <td>
                                <div class="d-flex no-block align-items-center">
                                    <div class="m-r-10"><img src="{{ $agencyAgent->avatar }}" alt="user"
                            ...
                            <td>
                                <select>
                                    @foreach ($roles as $rol)
                                        <option onclick='app.rolPrivileges(this)' value='{{ $rol }}'
                                            data-id='{{ $agencyAgent->id }}'
                            ...
                            <td>
                                @if (!$agencyAgent->active)
                                    <a href="javascript:void(0)" data-container="body" title="Resend Activation Email"
                                        data-toggle="popover" data-placement="right" data-trigger="focus"
                                        data-content="This account is still inactive.<br />
                                <a href='javascript:void(0)' onclick='app.resendActivationEmail(this)' data-id='{{ $agencyAgent->id }}' >Resend Activation Email</a>"
                                        class="fas fa-user-circle text-warning control-icon active-icon"
                                        data-agent-container="{{ $agencyAgent->id }}"></a>
                                @else
    ...
    @if (session()->get('totalSubmissions-' . $agency->id))
        ...
        @if (isset($totalCounts['Submitted']))
            ...
            <div class="progress">
                <div class="progress-bar bg-success" role="progressbar" aria-valuenow="80" aria-valuemin="0"
                    aria-valuemax="100" style="width:{{ $submittedPercent }}%; height:6px;"></div>
            </div>
        @endif
        ...
        @if (isset($totalCounts['Updated']))
            <h5 class="m-t-30">Incompelet Applications
                <span
                    class="pull-right">{{ $updatedPercent = floor(($totalCounts['Updated'] / $totalApplications) * 100) }}%</span>
            </h5>
            <div class="progress">
                <div class="progress-bar bg-orange" role="progressbar" aria-valuenow="90" aria-valuemin="0"
                    aria-valuemax="100" style="width:{{ $updatedPercent }}%; height:6px;"></div>
            </div>
        @endif
        ...
        @if (isset($totalCounts['Started']))
            <h5 class="m-t-30">Initiated Applications
                <span
                    class="pull-right">{{ $startedPercent = floor(($totalCounts['Started'] / $totalApplications) * 100) }}%</span>
            </h5>
            <div class="progress">
                <div class="progress-bar bg-danger" role="progressbar" aria-valuenow="90" aria-valuemin="0"
                    aria-valuemax="100" style="width:{{ $startedPercent }}%; height:6px;"></div>
            </div>
        @endif
    @endif
