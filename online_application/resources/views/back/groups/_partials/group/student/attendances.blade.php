@if ( $attendances_count = $studentAttendances->count() )
    <table id="group_student_attendances_table" class="table responsiveDataTable display new-table nowrap w-100 responsive dataTable">
        <thead>
            <tr>
                <th>{{__('Date')}}</th>
                <th>{{__('Classroom')}}</th>
    ...
    <script>
        $('#group_student_attendances_table').DataTable({
            "searching": false,
            "lengthChange": false,
            "columnDefs": [{
                "targets": 3,
