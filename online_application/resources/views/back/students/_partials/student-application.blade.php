        ...
        <div class="py-2 pl-2 pr-0 row application-header justify-content-between pl-md-4" data-parent="#accordion_application_action" href="#application-{{$submission->application->id}}">
            ...
            <div class="col-12 col-md-7">
                <div class="d-flex">
                    ...
                    @php
                        $statusesList = [];
                        $statuses = $submission->statuses->toArray();
                        foreach ($statuses as $status){

                            $at = isset($status['properties']['submitted_at']) ? $status['properties']['submitted_at'] : $status['updated_at'];
                            $at = is_array($at) ? $at['date'] : $at;
                            $statusesList[strtotime($at)] = $status['status'];
                        }
                        ksort($statusesList);
                    @endphp
                    <div class="af-column-title">
                        ...
                        <div class="application-statuses d-flex flex-wrap mb-3 mb-md-0">
                            <span class="pr-3"><i class="icon-clock text-secondary"></i> {{__('Created')}}: {{ $submission->created_at->translatedFormat('F j, Y') }}</span>

                            <span class="pr-3"><i class="icon-refresh text-secondary"></i> {{__('Updated')}}: {{ $submission->updated_at->translatedFormat('F j, Y') }}</span>

                            <span><i class="icon-info"></i> {{__('Step')}}: {{ isset($submission->properties['step']) ? $submission->properties['step'] : "N/A" }}</span>
                ...
                <div class="row w-100 h-100 align-items-center justify-content-between">
                    ...
                    <div class="col-3 col-md-3 mt-2 px-0">
                        <div class="btn-group more-optn-group float-right is-circle">
                            <button type="button"
                                class="btn btn-outline-secondary dropdown-toggle dropdown-toggle-split ti-more-alt is-circl flat-btn"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            </button>
                            <div class="dropdown-menu">

                                @if(!in_array('Approved' , $statuses) && $canEdit)

                                    <a href="javascript:void(0)"
                                @if($canEdit)
                                    onclick="app.approveStudentSubmission({{$submission->id}} , {{$submission->student->id}} , this)"
                                @endif
                                    class="dropdown-item" data-toggle="tooltip" data-placement="top" data-original-title="{{__('Approve Application')}}">
                                    <i class="mr-1 icon-check"></i><span class="pl-2 icon-text">{{__('Approve')}}</span>
                                    </a>
