 ...
 * @version    $Id: Xfa.php 1854 2023-04-28 05:57:01Z jan.slabon $
 */
...
class SetaPDF_FormFiller_Xfa
{
    ...
    public function isDynamic()
    {
        ...
        $hasSignature = SetaPDF_Core_Type_Dictionary_Helper::getValue($acroFormDict, 'SigFlags', 0, true) & 0x1;
        if (!$hasSignature) {
            return false;
        }

        foreach ($fields as $field) {
            $fieldDict = SetaPDF_Core_Type_Dictionary::ensureType(
                $this->_formFiller->getDocument()->ensureObject($field)
            );

            $fieldType = SetaPDF_Core_Type_Dictionary_Helper::resolveAttribute($fieldDict, 'FT');
            if (!$fieldType instanceof SetaPDF_Core_Type_Name || $fieldType->getValue() !== 'Sig') {
                return false;
            }
        }
