 ...
 * @version    $Id: AbstractField.php 1855 2023-04-28 10:34:15Z jan.slabon $
 */
...
abstract class SetaPDF_FormFiller_Field_AbstractField
{
     ...
     * @var bool
     */
    protected $_appearanceCreated = false;
    ...
    public function cleanUp()
    {
        ...
        $this->_font = null;
        if ($this->_appearanceCanvas !== null) {
    ...
    public function getAppearanceCanvas()
    {
        if ($this->_appearanceCanvas === null) {
            ...
            $this->_appearanceCanvas = new SetaPDF_Core_Canvas(SetaPDF_Core_XObject::get($n, 'Form'));
        }
     ...
     * @return array
     */
    protected function _getBorderWidthAndStyle()
    {
        $annotation = $this->getAnnotation();
        $appearanceCharacteristics = $annotation->getAppearanceCharacteristics();

        $borderStyleObject = $annotation->getBorderStyle();
        $borderWidth = 0;
        $borderStyle = SetaPDF_Core_Document_Page_Annotation_BorderStyle::SOLID;

        if ($borderStyleObject) {
            $borderStyle = $borderStyleObject->getStyle();
            $borderWidth = $borderStyleObject->getWidth();
        }

        if (SetaPDF_Core::isZero($borderWidth) && $appearanceCharacteristics && $appearanceCharacteristics->getBorderColor() !== null) {
            $borderWidth = 1;
        }
     ...
     *
     * @param null|SetaPDF_Core_Canvas $canvas
     * @return SetaPDF_Core_Canvas
     */
    protected function _recreateAppearance($circle = false, $canvas = null)
    {
        if ($canvas === null) {
        ...
        $rotation = $appearanceCharacteristics
            ...
            : 0;
        if (SetaPDF_Core::isNotZero($rotation)) {
            ...
            $nDictionary->offsetSet('Matrix', new SetaPDF_Core_Type_Array([
        ...
        // Set the BBox
        $nDictionary->offsetSet('BBox', new SetaPDF_Core_Type_Array([
        ...
        if (
            ...
            $colorLtValue = 1; //' 1 g';
            if ($borderStyle === SetaPDF_Core_Document_Page_Annotation_BorderStyle::INSET) {
                $colorLtValue = .5;
            } // ' 0.5 g';
            ...
            if (
                ...
                $borderStyle === SetaPDF_Core_Document_Page_Annotation_BorderStyle::BEVELED && $backgroundColor
            ) {
            ...
            if ($circle === true) {
                SetaPDF_Core_DataStructure_Color_Gray::writePdfString($canvas, $colorLtValue);
                $canvas->saveGraphicState();
                // clip
                $size = min($width, $height);
                $canvas->path()
                    ->moveTo($width / 2 - $size, $height / 2 - $size)
                    ->lineTo($width / 2 - $size, $height / 2 + $size)
                    ->lineTo($width / 2 + $size, $height / 2 + $size)
                    ->clip()
                    ->endPath();
        ...
        if ($borderColor) {
            ...
            // Dashed
            if ($borderStyle === SetaPDF_Core_Document_Page_Annotation_BorderStyle::DASHED) {
                $canvas->path()->setDashPattern($borderStyleObject->getDashPattern());
            }

            // Draw border
            // NOT underline
            if ($borderStyle !== SetaPDF_Core_Document_Page_Annotation_BorderStyle::UNDERLINE || $circle) {
                if ($circle === true) {
                    $canvas->draw()->circle(
                        $width / 2,
                        $height / 2,
                        (min($width, $height) - $borderWidth) / 2
                    );
                } else {
                    $canvas->draw()->rect(
                        $borderWidth * .5,
        ...
        if ($fonts->offsetExists($fontName)) {
            ...
            */
            ...
            if (!$defaultFonts->offsetExists($fontName)) {
                ...
                if (isset($aliases[$fontName])) {
                    $fontName = $aliases[$fontName];
                }
     ...
     *
     * @param boolean $recreateAppearanceIfNeedAppearancesIsSet Defines whether the field appearance is re-created if
     *                                                          the NeedAppearances flag is set.
     *
     * @throws SetaPDF_Core_Type_Exception
     * @throws SetaPDF_Core_Type_IndirectReference_Exception
     * @see SetaPDF_FormFiller_Field_AbstractField::delete()
     */
    public function flatten($recreateAppearanceIfNeedAppearancesIsSet = false)
    {
        $this->_preFlatten();

        $formFiller = $this->getFields()->getFormFiller();
        if (
            $recreateAppearanceIfNeedAppearancesIsSet &&
            $this->_appearanceCreated === false &&
            $formFiller->isNeedAppearancesSet()
        ) {
            $this->recreateAppearance();
        }
        ...
        try {
            ...
            if ($nStreamRef !== false && $nStreamRef->getValue() instanceof SetaPDF_Core_Type_IndirectObjectInterface) {
                // if this object was not observed before, force an observation!
                $nStreamRef->getValue()->observe();

                $appearance = new SetaPDF_Core_XObject_Form($nStreamRef);

                // ensure that a BBox is available...
                try {
                ...
                // if no BBox is available re-create it:
                if (!isset($bbox)) {
                    /** @var SetaPDF_Core_Type_Stream $xObjectStream */
                    $xObjectStream = $appearance->getIndirectObject()->ensure();
                    $xObjectStream->getValue()->offsetSet(
                        'BBox',
                        new SetaPDF_Core_Type_Array([
                            new SetaPDF_Core_Type_Numeric(0),
                            new SetaPDF_Core_Type_Numeric(0),
                            new SetaPDF_Core_Type_Numeric($this->getAnnotation()->getWidth()),
                            new SetaPDF_Core_Type_Numeric($this->getAnnotation()->getHeight())
                        ])
                    );
                }

                if ($appearance->getWidth() > 0 && $appearance->getHeight() > 0) {
                    $rect = $this->getAnnotation()->getRect()->getRectangle();
    ...
    public function delete()
    {
        ...
        while ($currentObject->getValue()->offsetExists('Parent')) {
            ...
            if ($currentObject->getValue()->offsetExists('Kids')) {
                ...
                foreach ($kids as $key => $value) {
                    if (!($value instanceof SetaPDF_Core_Type_IndirectReference)
                        || $lastObject->getObjectId() === $value->getObjectId()) {
        ...
        foreach ($objectsToDelete AS $objectToDelete) {
            ...
            foreach ($fieldsArray AS $key => $value) {
                if (!($value instanceof SetaPDF_Core_Type_IndirectReference)
                    || $objectToDelete->getObjectId() === $value->getObjectId()) {
        ...
        if ($removeFieldsEntry && $fieldsArray !== false) {
            foreach ($fieldsArray AS $key => $value) {
                if (!($value instanceof SetaPDF_Core_Type_IndirectReference)
                    || $currentObject->getObjectId() === $value->getObjectId()) {
                    $fieldsArray->offsetUnset($key);
                    break;
                }
            }
