...
class SetaPDF_FormFiller_Field_Formatter
{
    ...
    public static function makeDate($value, $format = null)
    {
        ...
        if ($format !== null) {
            ...
            foreach (self::_tokenizeFormat($format) as $token) {
                switch ($token) {
                    ...
                    case 'm':
                    case 'mm':
                        $regex .= '(?<month>(1[012]|0?[1-9]))';
                        break;
                    case 'mmm':
                    case 'mmmm':
                        $regex .= '(?<month>[^' . $delimiter .']+)';
                        break;
                    case 'yy':
                        $regex .= '(?<year>\d{2,4})'; // as an input also 4 digits are supported
                        break;
                    case 'yyyy':
                        $regex .= '(?<year>\d{3,4})';
                        break;
                    case 'h':
                    case 'HH':
                        $regex .= '(?<hour>(0?[0-9]|[1-5]\d))';
                        break;
                    case 'M':
                    case 'MM':
                        $regex .= '(?<minutes>(0?[0-9]|[1-5]\d))';
                        break;
                    case 's':
                    case 'ss':
                        $regex .= '(?<seconds>(0?[0-9]|[1-5]\d))';
                        break;
                    case 't':
                    case 'tt':
                        $regex .= '(?<amOrPm>[^' . $delimiter .']+)';
                        break;
                    default:
                        if (preg_match('~^[' . $delimiter .']+$~', $token)) {
                            $regex .= '[' . $delimiter .']+';
                        } else {
                            if ($token[0] === '\\') {
        ...
        $parts = preg_split('~([' . $delimiter . '])~', $value, -1, PREG_SPLIT_DELIM_CAPTURE);
