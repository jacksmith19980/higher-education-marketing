 ...
 * @version    $Id: Tree.php 1866 2023-07-05 09:17:13Z jan.slabon $
 */
    ...
    implements SetaPDF_Core_DataStructure_DataStructureInterface
{
    ...
    public function get($key, $className = null)
    {
        if (!is_int($key) && !is_string($key)) {
            throw new InvalidArgumentException('Keys for tree structures can only be of type int or string.');
        }
            ...
            while (($node = array_shift($stack)) !== null) {
                ...
                if ($keys) {
                    ...
                    for ($i = 0, $n = $keys->count(); $i < $n; $i += 2) {
                        ...
                        // By the specification only "Integer" and StringValues are possible.
                        // But sometimes Name objects are used, too.
                        if (
                            !$key instanceof SetaPDF_Core_Type_Numeric &&
                            !$key instanceof SetaPDF_Core_Type_StringValue &&
                            !$key instanceof SetaPDF_Core_Type_Name
                        ) {
                            continue;
                        }
     ...
     *
     * @param int|string|SetaPDF_Core_Type_ScalarValue $key Depends on the implementation
    ...
    public function add($key, SetaPDF_Core_Type_AbstractType $value)
    {
        ...
        if (!is_int($keyValue) && !is_string($keyValue)) {
            throw new InvalidArgumentException('Keys in tree structures need to be an int or string value.');
        }
