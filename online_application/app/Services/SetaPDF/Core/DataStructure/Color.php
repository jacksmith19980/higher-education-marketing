 ...
 * @version    $Id: Color.php 1832 2023-02-14 11:09:14Z jan.slabon $
 */
...
class SetaPDF_Core_DataStructure_Color
{
    ...
    public static function writePdfStringByComponents(SetaPDF_Core_WriteInterface $writer, $components, $stroking = true)
    {
        if (!is_array($components)) {
            ...
            } elseif (is_string($components) && $components !== '' && $components[0] === '#') {
    ...
    public static function createByComponents($components)
    {
        if (!$components instanceof SetaPDF_Core_Type_Array) {
            if (is_string($components) && $components !== '' && $components[0] === '#') {
                $components = self::hexToRgb($components);
            }
            ...
            foreach ($components AS $component) {
                if ($component instanceof SetaPDF_Core_Type_Numeric) {
                    ...
                    if (!is_numeric($component)) {
                        throw new InvalidArgumentException(
                            'The $component value is not a numeric value.'
                        );
                    }
    ...
    public static function hexToRgb($hex)
    {
        ...
        if (\preg_match('/([^A-F0-9])/i', $hex)) {
            throw new InvalidArgumentException('Argument needs to be a valid hex value.');
        }
        ...
        if ($len !== 6) {
            throw new InvalidArgumentException('Only hex values of 3 or 6 characters are allowed.');
        }
