 ...
 * @version    $Id: CorruptedCrossReferenceTable.php 1849 2023-04-26 08:12:13Z jan.slabon $
 */
    ...
    implements SetaPDF_Core_Parser_CrossReferenceTable_CrossReferenceTableInterface
{
    ...
    protected function _screen()
    {
        ...
        $maxStart = $reader->getTotalLength();
        ...
        while (($buffer = $reader->getBuffer()) !== '') {
            ...
            $start += $bufferLen / 2;
            $start = min(max($start, $newStart), $maxStart);
            $reader->reset($start, $bufferLen);
        }
    ...
    {
        if (($pos = strpos($buffer, 'trailer')) !== false) {
    ...
    public function ensureTrailer()
    {
        ...
        if ($root === null) {
            ...
            if (SetaPDF_Core_Type_Dictionary_Helper::keyHasValue(
                SetaPDF_Core_Type_Dictionary::ensureType($tmpRoot), 'Type', 'Catalog'
            )) {
                $root = $tmpRoot;
            }
        }
