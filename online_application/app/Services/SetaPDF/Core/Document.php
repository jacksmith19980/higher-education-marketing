 ...
 * @version    $Id: Document.php 1816 2023-01-30 14:45:38Z jan.slabon $
 */
    ...
    implements SplObserver, SetaPDF_Core_WriteInterface, SetaPDF_Core_Type_Owner
{
     ...
     * @var string
     */
    protected static $_instanceIdentPrefix = null;
     ...
     */
    public static function loadByFilename
    (
        $filename,
        SetaPDF_Core_Writer_WriterInterface $writer = null,
        $className = 'SetaPDF_Core_Document'
     ...
     * @return SetaPDF_Core_Document
     */
    public static function loadByString
    (
        $string,
        SetaPDF_Core_Writer_WriterInterface $writer = null,
        $className = 'SetaPDF_Core_Document'
     ...
     *
     * @param bool $compressXref Pass true to enforce that the cross-reference will be compressed. Pass false to
    ...
    public function setCompressXref($compressXref)
    {
        ...
        $this->_compressXref = (bool)$compressXref;
    ...
    public function save($method = true)
    {
        ...
        if (self::SAVE_METHOD_UPDATE === $method &&
            ...
            if (count($this->_changedObjects) > 0 || $this->_trailerChanged || $this->_xref->offsetsUpdated()) {
                $this->_writer->write("\n");
                $this->_writeFileBody();
    ...
    protected function _updateFileIdentifier()
    {
        ...
        if ($newId === null) {
            ...
            $info = $this->getInfo()->getDictionary();
            ...
            $this->setNewFileIdentifier($newId);
            return $newId;
        }

        $id = SetaPDF_Core_Type_Dictionary_Helper::getValue($this->_trailer, 'ID');
        if ($id === null || !$id instanceof SetaPDF_Core_Type_Array || $id->count() !== 2) {
        ...
        // Move array to a direct value if it is an indirect reference
        if ($this->_trailer->getValue('ID') instanceof SetaPDF_Core_Type_IndirectReference) {
            $this->_trailer->offsetSet('ID', $id);
        }
