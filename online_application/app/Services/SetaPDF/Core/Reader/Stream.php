 ...
 * @version    $Id: Stream.php 1849 2023-04-26 08:12:13Z jan.slabon $
 */
    ...
    implements SetaPDF_Core_Reader_ReaderInterface
{
    ...
    protected function _setStream($stream)
    {
        ...
        // custom stream wrappers are declared as seekable but aren't always really seekable, so we have to test it
        if (fseek($stream, 0) === -1) {
            throw new InvalidArgumentException(
                'Given stream is not seekable!'
            );
    ...
    public function getTotalLength()
    {
        if ($this->_totalLength === null) {
            ...
            if ($stat === false) {
                throw new SetaPDF_Core_Reader_Exception('Cannot get file information through fstat() on stream handle.');
            }
    ...
    public function reset($pos = 0, $length = 200)
    {
        ...
        if (fseek($this->_stream, $pos) === -1) {
            // custom stream wrappers are declared as seekable but aren't always really seakable
            throw new InvalidArgumentException(
                sprintf('Unable to reset stream to position %d.', $pos)
            );
        }
