 ...
 * @version    $Id: Names.php 1868 2023-08-25 06:35:57Z jan.slabon $
 */
...
class SetaPDF_Core_Document_Catalog_Names
{
    ...
    public function getTree($name, $create = false)
    {
        ...
        if (!$names instanceof SetaPDF_Core_Type_Dictionary) {
            return null;
        }

        $treeRootDict = SetaPDF_Core_Type_Dictionary_Helper::getValue($names, $name);

        if (!$treeRootDict  instanceof SetaPDF_Core_Type_Dictionary) {
            if ($create === false) {
                return null;
            }
    ...
    public function getNamesDictionary($create = false)
    {
        if ($this->_namesDictionary === null) {
            ...
            if (!$namesDict instanceof SetaPDF_Core_Type_Dictionary) {
                if ($create === false) {
                    return null;
                }
