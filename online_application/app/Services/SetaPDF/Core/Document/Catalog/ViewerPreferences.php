 ...
 * @version    $Id: ViewerPreferences.php 1869 2023-08-25 06:50:54Z jan.slabon $
 */
...
class SetaPDF_Core_Document_Catalog_ViewerPreferences
{
    ...
    public function setNonFullScreenPageMode($name = self::NON_FULL_SCREEN_PAGE_MODE_USE_NONE)
    {
        ...
        if (!in_array($name, [
            ...
            throw new InvalidArgumentException(sprintf('Invalid page mode for full-screen mode (%s).', $name));
        }
    ...
    public function setDirection($name)
    {
        ...
        if (!in_array($name, [self::DIRECTION_L2R, self::DIRECTION_R2L])) {
            throw new InvalidArgumentException(sprintf('Invalid value for predominant reading order (%s).', $name));
        }
    ...
    public function setDuplex($name)
    {
        ...
        if (!in_array($name, [
            self::DUPLEX_SIMPLEX,
            self::DUPLEX_FLIP_SHORT_EDGE,
            self::DUPLEX_FLIP_LONG_EDGE
        ])) {
            throw new InvalidArgumentException(sprintf('Invalid value for paper handling option (%s).', $name));
        }
