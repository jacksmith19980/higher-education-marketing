...
extends SetaPDF_Core_Document_Page_Annotation_Markup
{
    ...
    public static function createAnnotationDictionary($rect)
    {
        return parent::_createAnnotationDictionary(
            $rect,
            SetaPDF_Core_Document_Page_Annotation::TYPE_CARET
        );
    }
    ...
    {
        $symbol = SetaPDF_Core_Type_Dictionary_Helper::getValue($this->_annotationDictionary, 'Sy');
        if (!$symbol instanceof SetaPDF_Core_Type_Name) {
    ...
    {
        $differencesRect = SetaPDF_Core_Type_Dictionary_Helper::getValue($this->_annotationDictionary, 'RD');
        if (!$differencesRect instanceof SetaPDF_Core_Type_Array) {
            return null;
        }
    ...
    {
        if (SetaPDF_Core::isZero(array_sum($differencesRect->toPhp()))) {
