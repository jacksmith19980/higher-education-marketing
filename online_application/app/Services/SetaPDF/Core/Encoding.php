 ...
 * @version    $Id: Encoding.php 1850 2023-04-26 10:27:14Z jan.slabon $
 */
  ...
  /* PDF specific predefined encodings */
    ...
    public static function convert($string, $inEncoding, $outEncoding)
    {
        ...
        if (count($_outEncoding) > 1) {
            $ignore = in_array('IGNORE', $_outEncoding, true);
            $translit = in_array('TRANSLIT', $_outEncoding, true);
        } else {
     ...
     * @return string
     */
    public static function fromUtf16Be($table, $string, $ignore = false, $translit = false, $substituteChar = "\x1A")
    {
        $newString = '';
            ...
            for ($i = 0, $len = strlen($string); $i < $len; $i++) {
                ...
                if ($res === false) {
                    foreach ($table AS $utf16 => $char) {
                        ...
                        if (in_array($string[$i], $char, true)) {
                            $res = $utf16;
                            break;
                        }
                    }
    ...
    public static function strSplit($string, $encoding = 'UTF-8')
    {
        ...
        if (self::$library === 'mb' && function_exists('mb_str_split')) {
