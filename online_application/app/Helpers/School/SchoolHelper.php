...
class SchoolHelper
{
    public static function school($uid = null)
    {
        if(!$uid)
        {
            $uid = session('tenant');
        ...
        return School::byUuid($uid)->first();
    ...
    public static function renderSchoolLogo($school = null, $settings = null)
    {
        if (!$school->name) {
            return '<h4>'.config('app.name', 'Online Application').'</h4>';
        }

        if ( ($settings && !empty($settings)) && ( isset($settings['branding']['logo']) && !self::isPng($settings['branding']['logo']) )) {

            if (isset($settings['branding']['logo'])) {
                return '<img class="school-logo"
                    src="'.Storage::disk('s3')->temporaryUrl($settings['branding']['logo']['path'] , Carbon::now()->addMinutes(5)).'" />';
            }
        } else {
    ...
    protected static function isPng($logo){
        return false;
        $tmp = explode(".", $logo['path']);
        return $tmp[count($tmp) - 1] == 'png';
    }

    protected static function convertPNGtoJPG($logo)
    {
        $filePath = Storage::disk('s3')->temporaryUrl($logo['path'], Carbon::now()->addMinutes(5));
    ...
    public static function getSchedulesList()
    {
    ...
    public static function renderFavIcon($school = null, $settings = null)
    {
        if (! $school) {
            return asset('media/images/favicon.png');
        }

        if ($settings && ! empty($settings)) {
            if (isset($settings['branding']['icon'])) {
                return Storage::disk('s3')->temporaryUrl($settings['branding']['icon']['path'],
    ...
    public static function generatePassword($_len = 10) {
    ...
    public static function getStartNewApplicationLink()
    {
        if(!$school = School::whereSlug(request()->school)->first()){
            return '/';
        }
        ...
        return route('new.applications', $school);
    }
    ...
    {
        $school = self::school();
        return $school->users->pluck('name' , 'id')->toArray();
    }
}
