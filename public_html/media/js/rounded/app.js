var app = {
    ...
    dateTimePicker: function () {
        $('body').on('focus', ".datepicker-autoclose", function () {
            ...
            var options = {
                ...
                /* format: {
                    ...
                    toDisplay: function (date, format, language) {
                        ...
                        return moment(d).format(display)
                    },
            ...
            if ($(this).data('start-date')) {
                options.startDate = moment($(this).data('start-date'), 'YYYY-MM-DD').format(display);
            }

            if ($(this).data('end-date')) {
                options.endDate = moment($(this).data('end-date'), 'YYYY-MM-DD').format(display);
            }
    ...
    jsSignature: function () {
        ...
        $('.canvas-wrapper').each(function () {
            ...
            app.startSignaturePad(canvas , data);
    ...
    startSignaturePad: function (canvas , data) {
        ...
        app.appAjax('POST', data, route, 'data').then(function (data) {
            ...
            if (data.response == 'success' && data.status == 200) {
                ...
                if (data.extra.type != null) {
                    ...
                    $('input.' + fileHolderName).val("").change();
    ...
    wizardForm: function () {
        ...
        var stepsForms = $(".validation-wizard").steps({
            ...
            titleTemplate: '<span class="step">#index#</span> <span class="step_title">#title#</span>',
            saveState: true,
            ...
            onStepChanged: function (event, currentIndex, priorIndex) {
                ...
                if ($('section.current .review-page').length) {
                    app.renderReviewPage($(event.target))
                }
                ...
                /*  if($(event.target).data('review') == (currentIndex + 1) ){
    ...
    renderReviewPage: function (el) {
        el = $('div.review-page')[0];
        var url = $(el).data('route');
        console.log(url);
        var data = {}
        app.appAjax('POST', data, url).then(function (response) {
            if (response.status == 200) {
                console.log('renderReviewPage');
    ...
    processSmartField: function (el) {
        ...
        //get referance field
        var ref = $('[name="' + el.attr('data-reference') + '"]');
    ...
    processRepeatedFields: function (fields, action) {
        /* if (typeof fields == 'undefined') {
            return;
        }
        for (let i = 0; i < fields.length; i++) {
            const element = $('[name="' + fields[i] + '"]');
            console.log("element");
            console.log(element);
            if (action == 'disable') {
                element.attr('disbaled', 'disbaled');
            } else {
    ...
    showSmartField: function (el, field, ref, ref_value, operator, value) {
        var repeatedFields = el.data('repeater-fields');
        if (operator == 'equal') {
            if (ref_value == value) {
                el.show();
                field.removeAttr("disabled");
                el.find($('.repeated_field')).removeAttr("disabled");
            } else {
                el.hide();
                app.resetField(field);
                field.attr("disabled", "disabled");
                el.find($('.repeated_field')).attr("disabled", "disabled");
            }
    ...
    repeatFields: function (el, counter) {
        ...
        $(clone).find('input, select').each(function () {
            ...
            var name = originalName + "[" + ID + "]";

            if ($(this).data("sync-field")) {
                var syncField = $(this).data("sync-field");
                var newSync = syncField.replace('[1]', '[' + ID + ']');
                $(this).attr("data-sync-field", newSync).change();
            }
        ...
        $(clone).find('.field_wrapper').each(function () {
            ...
            if (typeof ref != 'undefined') {
                ...
                $(this).attr('data-reference', new_ref);
                $(this).attr('data-field', new_field);
                $(this).attr('style', '');
    ...
    syncField: function () {
        if ($("[data-sync]").length > 0) {
            ...
            $("[data-sync]").each(function () {
                ...
                var mainField = field.attr("data-sync-field");
                var target = field.attr('data-sync-target');
                var source = field.attr('data-sync-source');
                var route = field.attr('data-route');


                $('[name="' + mainField + '"]').change(function () {
                    app.getSyncedData(this, field, target, source, route, fieldValue, mainField);
                });
                if (!$(this).val()) {
                    var el = ('[name="' + mainField + '"]');
                    var fieldValue = $(this).data('field-value');
                    app.getSyncedData(el, field, target, source, route, fieldValue, mainField);
                }
            });
        }
    },
    getSyncedData(el, field, target, source, route, fieldValue, mainField) {

        // Return the field name is not matching the main field name
        if ($(el).attr('name') != mainField) {
            return;
        }
        ...
        var formData = $('#steps-uid-0').serialize();
        var data = {
            value: value,
            target: target,
            source: source,
    ...
    searchDatesAddons: function (itemSelected, route) {
        ...
        $.ajax({
            ...
            app.getCustomFields(data.extra.customFieldsHtml, itemSelected);
        });
    },
    ...
    getCustomFields: function (customFields, itemSelected, hash = 0) {
        if (customFields !== 0) {
            if (hash === 0) {
                $('.custom-fields').append(customFields);
            } else {
                $('.custom-fields-' + hash).append(customFields);
            }
        }
    },
    ...
    searchPrograms: function (el, object, route) {
        ...
        campus = $('select[name="' + fieldName + '[campus]"]').val(),
        programType = $('select[name="' + fieldName + '[programType]"]').val();
    ...
    searchProgramDatesAddons: function (itemSelected, route) {
        ...
        app.cleanPrograDivsContainers();
        var field   = program.data('field');
        var campus = $('.program-campus-field').val();
        $.ajax({
            url: route,
            type: "get",
            dataType: 'json',
            ...
            app.getProgramCustomFields(data.extra.customFieldsHtml, itemSelected);

            $(document).trigger("programChanged");
        });
    },
    ...
    getProgramCustomFields: function (customFieldsHtml, itemSelected) {
        if (customFieldsHtml !== "") {
            $('.custom-fields').append(customFieldsHtml);
        }
    },
    ...
    syncProgramDateOnCart: function (itemSelected) {
        var syncroute = $(itemSelected).data('syncroute');
        var fieldName = $(itemSelected).data('field-name');
        ...
        //(method, data, route, dataType) {
        app.appAjax('POST', data, syncroute).then(function (data) {
            //data.cart.data.programs.start_date
            $("[name='"+fieldName+"[start_date]']").val(data.cart.data.programs.start_date);
            $("[name='" + fieldName + "[end_date]']").val(data.cart.data.programs.end_date);
            $("[name='" + fieldName + "[schedule]']").val(data.cart.data.programs.date_schudel_label);
